...
void prob_initdata (int i, int j, int k, Array4<Real> const& state, GeometryData const& geomdata, ProbClosures const& pcls, const ProbParm& pparm) {
  ...
  // Turbulent profiles
  Real T = 677.04110513 + 255.46817*pow(yw,0.5) - 176.38967315*exp(-3081.95957181*yw)*1.025;
  T = std::max(T,50.0);
  T = std::min(T,1200.0);
  Real ux = 581.54524091 + 2687.84321585*pow(yw,0.5) - 593.73493364* exp(-2008.87100276*yw);
