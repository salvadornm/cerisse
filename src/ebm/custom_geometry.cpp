...
#if AMREX_SPACEDIM == 3
void Combustor::build(const Geometry& geom, const int max_coarsening_level)
{
  ...
  EB2::PlaneIF farwall({AMREX_D_DECL(fwl, 0., 0.)}, {AMREX_D_DECL(1., 0., 0.)});
  ...
  auto ramp = EB2::makeIntersection(
    EB2::PlaneIF({pl1pt[0], pl1pt[1], 0.}, {0., -1., 0.}),
    EB2::PlaneIF({pl2pt[0], pl2pt[1], 0.}, {pl2nm[0], pl2nm[1], 0.}),
    EB2::PlaneIF({pl3pt[0], pl3pt[1], 0.}, {1., 0., 0.}));
...
void ConvergingNozzle::build(const Geometry& geom, const int max_coarsening_level)
{
  ...
  EB2::CylinderIF main(0.5 * d_inlet, 0,
                       {AMREX_D_DECL(static_cast<Real>(0.5 * l_inlet), 0, 0)}, true);

  Real slope_nozzle = (0.5 * d_inlet - 0.5 * d_exit) / l_nozzle;
  Real norm = -1.0 / slope_nozzle;
  Real nmag = std::sqrt(1 + 1 / (norm * norm));
  EB2::PlaneIF nozzle_plane(
    {AMREX_D_DECL(0, 0, 0)},
    {AMREX_D_DECL(static_cast<Real>(1.0 / nmag), slope_nozzle / nmag, 0.0)}, true);
  auto nozzle = EB2::translate(
    EB2::rotate(EB2::lathe(nozzle_plane), 90 * M_PI / 180, 1),
    {AMREX_D_DECL(l_inlet + static_cast<Real>(0.5 * d_inlet / slope_nozzle), 0, 0)});

  EB2::CylinderIF exit(0.5 * d_exit, 0, {AMREX_D_DECL(l_inlet + l_nozzle, 0, 0)},
                       true);
  auto nozzle_exit = EB2::makeIntersection(nozzle, exit);

  auto polys = EB2::makeUnion(main, nozzle_exit);
  auto gshop = EB2::makeShop(polys);
  EB2::Build(gshop, geom, max_coarsening_level, max_coarsening_level, 6, false);
}
#endif

void Triangles::build(const Geometry& geom, const int max_coarsening_level)
{
  ...
  amrex::Vector<Array<Real, AMREX_SPACEDIM>> alltri(npts_in_tri * max_tri);
  ...
  for (int itri = 0; itri < num_tri; itri++) {
    Array<Real, AMREX_SPACEDIM> point{AMREX_D_DECL(0.0, 0.0, 0.0)};

    for (int ipt = 0; ipt < npts_in_tri; ipt++) {
      std::string pointstr =
        "tri_" + std::to_string(itri) + "_point_" + std::to_string(ipt);
      amrex::Vector<Real> vecpt;
      pp.getarr(pointstr.c_str(), vecpt, 0, AMREX_SPACEDIM);
      for (int dir = 0; dir < AMREX_SPACEDIM; dir++) { point[dir] = vecpt[dir]; }
  ...
  for (int itri = 0; itri < max_tri; itri++) {
    ...
    impfunc_triangles[itri] = std::make_unique<
  ...
  auto alltri_IF = EB2::makeUnion(*impfunc_triangles[0], *impfunc_triangles[1],
                                  *impfunc_triangles[2], *impfunc_triangles[3],
                                  *impfunc_triangles[4]);
...
// {
//   // cylindrical domain
//   auto cchamber_1= EB2::CylinderIF(2.2,9, 2,{AMREX_D_DECL(2.2, 2.2, 9)}, true);
//   auto injector = EB2::CylinderIF(0.95, 9, 2, {AMREX_D_DECL(2.2, 2.2, 3)}, false);
