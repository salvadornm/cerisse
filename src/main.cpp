...
int main(int argc, char* argv[])
{
  ...
  if (max_step <= 0 || stop_time <= Real(0.0)) {
    amrex::Abort("Exiting because either max_step and/or stop_time is less than or "
                 "equal to 0.");
  }
  ...
  // Setup -------------------------------------------------------------------
  {
    ...
    EBM::eb.init(amr.Geom(amr.maxLevel()), amr.maxLevel(), amr.maxLevel());
#endif
  ...
  ParallelDescriptor::ReduceRealMax<Real>({timer_tot, timer_init, timer_advance},
