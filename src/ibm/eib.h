...
///
/// \brief eib_t is explicit geometry (triangulation based) immersed boundary method class. It holds an array of IBMultiFab, one for each AMR level; and it also holds the geometry
///
...
class eib_t {
 public:
  ...
  {
    bmf_a[lev] = new IBMultiFab<bool,gpData_t<eorder_tparm>>(bxa, dm, 2, cls_t::NGHOST);
  ...
  private:
    void read_geom()
    {
      ...
      for (int i = 0; i < ngeom; i++) {
        ...
        Print() << "Geometry (i=" << i << ") " << files_a[i] << " read"
                << std::endl;
        ...
        for (int ii = 0; ii < ngeom; ii++) {
          inout_fa[ii] = new inside_t(geom_a[ii]);
        }
