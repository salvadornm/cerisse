...
contains
  ...
  subroutine hyp_mol_gam_3d(q, qd_lo, qd_hi, &
    ...
    use cns_module, only : urho, umx, umy, umz, ueden, ueint, utemp, nvar, &
        qrho,qu,qv,qw,qp,qc,qeint,qtemp,qvar, smallp, smallr, actually_2D, &
        nvarsolve 
    ...
    call amrex_allocate ( dq, qtlo(1), qthi(1), qtlo(2), qthi(2), qtlo(3), qthi(3), 1, NVARSOLVE)
   
    call bl_proffortfuncstart_int(0)
    call slopex(q,qd_lo,qd_hi, &
        dq,qtlo,qthi, &
        lo(1),lo(2),loz(3),  &
        hi(1),hi(2),hiz(3),QVAR)
