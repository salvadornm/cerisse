...
contains
  ...
  subroutine cns_compute_dudt (lo,hi, dudt, utlo, uthi, &
    ...
    call amrex_allocate(fhx,lo(1),hi(1)+1,lo(2),hi(2)  ,lo(3),hi(3)  ,1,NVARSOLVE)
    call amrex_allocate(fhy,lo(1),hi(1)  ,lo(2),hi(2)+1,lo(3),hi(3)  ,1,NVARSOLVE)
    call amrex_allocate(fhz,lo(1),hi(1)  ,lo(2),hi(2)  ,lo(3),hi(3)+1,1,NVARSOLVE)
    call amrex_allocate(fdx,lo(1),hi(1)+1,lo(2),hi(2)  ,lo(3),hi(3)  ,1,NVARSOLVE)
    call amrex_allocate(fdy,lo(1),hi(1)  ,lo(2),hi(2)+1,lo(3),hi(3)  ,1,NVARSOLVE)
    call amrex_allocate(fdz,lo(1),hi(1)  ,lo(2),hi(2)  ,lo(3),hi(3)+1,1,NVARSOLVE)
    ...
    do n = 1, NVARSOLVE
    do k = lo(3)+1, hi(3)
      fhx(lo(1):hi(1)+1,lo(2):hi(2)  ,k,n) = fx(lo(1):hi(1)+1,lo(2):hi(2)  ,lo(3),n)
      fhy(lo(1):hi(1)  ,lo(2):hi(2)+1,k,n) = fy(lo(1):hi(1)  ,lo(2):hi(2)+1,lo(3),n)
    end do
    end do
    ! end snm
    ...
    ! old RHS rhs =  dF/dx + dF/dy + dF/dz (FV)
    ...
    ! SNM: add these lines to make it 2D
    if (actually_2D) then
       fz = 0.d0   
       dudt(lo(1):hi(1),lo(2):hi(2),lo(3):hi(3),UMZ) = 0.d0 
       do n = 1, 5
          do k = lo(3), hi(3)
             dudt(lo(1):hi(1),lo(2):hi(2),k,n) = dudt(lo(1):hi(1),lo(2):hi(2),lo(3),n)
             fx(lo(1):hi(1)+1,lo(2):hi(2)  ,k,n) = fx(lo(1):hi(1)+1,lo(2):hi(2)  ,lo(3),n)
             fy(lo(1):hi(1)  ,lo(2):hi(2)+1,k,n) = fy(lo(1):hi(1)  ,lo(2):hi(2)+1,lo(3),n)
          end do
       end do
  ...
  subroutine cns_eb_compute_dudt (lo,hi, dudt, utlo, uthi, &
    ...
    ! calculate hyperbolic fluxes with Embbeded Boundaries
   ...
   ! calculate diffusion coefficients viscoisty conductivity and so on..
   call bl_proffortfuncstart_int(8)
   call compute_diff_coef(q, qlo, qhi, lambda, mu, xi, clo, chi)   
   call bl_proffortfuncstop_int(8)

   ! compute  diffusion fluxes with Embbeded Boundaries
    ...
    fx(      fxlo(1):fxhi(1),fxlo(2):fxhi(2),fxlo(3):fxhi(3),1:NVARSOLVE) = &
         fhx(fxlo(1):fxhi(1),fxlo(2):fxhi(2),fxlo(3):fxhi(3),1:NVARSOLVE)
    fy(      fylo(1):fyhi(1),fylo(2):fyhi(2),fylo(3):fyhi(3),1:NVARSOLVE) = &
         fhy(fylo(1):fyhi(1),fylo(2):fyhi(2),fylo(3):fyhi(3),1:NVARSOLVE)
    fz(      fzlo(1):fzhi(1),fzlo(2):fzhi(2),fzlo(3):fzhi(3),1:NVARSOLVE) = &
         fhz(fzlo(1):fzhi(1),fzlo(2):fzhi(2),fzlo(3):fzhi(3),1:NVARSOLVE)

    fx(fxlo(1):fxhi(1),fxlo(2):fxhi(2),fxlo(3):fxhi(3),nvarsolve+1:nvar) = 0.d0
    fy(fylo(1):fyhi(1),fylo(2):fyhi(2),fylo(3):fyhi(3),nvarsolve+1:nvar) = 0.d0
    fz(fzlo(1):fzhi(1),fzlo(2):fzhi(2),fzlo(3):fzhi(3),nvarsolve+1:nvar) = 0.d0

    dm_as_fine = 0.d0

    ! divergence correction close to Embbeded Boundaries
    ...
    dudt(lo(1):hi(1),lo(2):hi(2),lo(3):hi(3),nvarsolve+1:nvar) = 0.d0
    call bl_proffortfuncstop_int(10)

     ! SNM (temporary) added lines to ensure 2D
    if (actually_2D) then
       fz = 0.d0   
       dudt(lo(1):hi(1),lo(2):hi(2),lo(3):hi(3),UMZ) = 0.d0 
       do n = 1, NVARSOLVE
