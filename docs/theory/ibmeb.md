...
The volume of the cells  is $$V= \phi h^3$$  (assuming an isotropic mesh) is proportional to the 'filled' fraction of each cell, $$\phi$$ . Based on this, the area of a face is also proportional to the mesh size, $$A_k = f_k h^2$$. The area can be expressed in terms of a ratio, $$f_k$$,  relative to the maximum possible (unobstructed) face area; this ratio equals 1 when the face is completely unobstructed
...
where the flux at the wall, $$F_{wall}$$ would be defined based on the boundary condition type.
...
\[1] Monal Patel, "Hypersonic Flows Around Complex Geometries with Adaptive Mesh Refinement and Immersed Boundary Method" [_PhD Thesis_, Imperial College London (2022)](https://doi.org/10.25560/104074)
