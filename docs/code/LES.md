...
This page explains how basic LES models are implemented in the  `LES_t` class, defined in the`LES.h` file.



The typical set-up is in `prob.h`
...
These parameters include sub-grid Prandtl number , Schmid number (LINK TO THEORY).
...
This start by computing the velociuty gradient tensor using central differences,
given the LES order.
...
schemes to sixth order.
If the sub-grid viscosity is to be calculated in the ghost points, for example to be added to the viscosity
the order of viscous term and LES cannot be  independent.
...
\left( \alpha_{2} \rhoD_{i+2} +  ... \alpha_{-2} \rhoD_{i-2} \right) 
\frac{\beta_{2}\phi_{i+2} + ... \beta_{2}\phi_{i+-2}}{\Delta x}
$$

where $$\alpha$$ and $$\beta$$ are interpolation and derivative face-centred weights respectively.
...
In general
$$
\mbox{half stencil viscous}  + \mbox{half stencil LE}  \leq 3
$$

Therefore, *without changing the default value of GHOST POINTS(3)*
...
NOTE: The number of GHOST POINTS can be by changing `indicies_t` in the `prob.h` to the more general
`indices_gent<>` passing the number of GHOST points as argument. Beware using this, as the number of GHOST points is used in many places, includi i/o subroutines.
