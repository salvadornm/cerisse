  ...
  Array4<const int> const& lev_mask, Real dt)
{
...
#endif
  {
    ...
    amrex::ParallelFor(bxg3, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
      eb_compute_div(
        i, j, k, blo, bhi, q, divc, AMREX_D_DECL(fx_in, fy_in, fz_in),
        AMREX_D_DECL(fx_out, fy_out, fz_out), flag, vfrac, bcent, coefs,
        AMREX_D_DECL(apx, apy, apz), AMREX_D_DECL(fcx, fcy, fcz), dxinv, *lparm,
  ...
  // Now do redistribution
  {
    ...
    int level_mask_not_covered = h_parm->level_mask_notcovered;
