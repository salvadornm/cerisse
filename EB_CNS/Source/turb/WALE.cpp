...
WALE::mu_T_cc (const int i, const int j, const int k,
               ...
               Real& mu_T)
{
  ...
  for (int m = 0; m < AMREX_SPACEDIM; m++) {
    for (int n = 0; n < AMREX_SPACEDIM; n++) {
      Sijmag += (0.5 * (dUdx[m][n] + dUdx[n][m])) * (0.5 * (dUdx[m][n] + dUdx[n][m])); // Sij*Sij
      Dijmag += (0.5 * (dUdx[m][n]*dUdx[m][n] + dUdx[n][m]*dUdx[n][m]) - Real(m == n) * divu * divu / 3) 
              * (0.5 * (dUdx[m][n]*dUdx[m][n] + dUdx[n][m]*dUdx[n][m]) - Real(m == n) * divu * divu / 3); // Dij*Dij
    }
  }

  Real Cw = std::sqrt(10.6) * Cs;
  mu_T = q(i, j, k, QRHO) * Cw * Cw * deltabar * deltabar 
          * std::pow(Dijmag, 1.5) / (std::pow(Sijmag, 2.5) + std::pow(Dijmag, 1.25));
}
 ...
 */
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void
WALE::mu_T_fc (const Real dudx, const Real dudy, const Real dudz, 
               ...
               Real& mu_T)
{
  ...
  Real Cw = std::sqrt(10.6) * Cs;
  mu_T = rho * Cw * Cw * deltabar * deltabar
         * std::pow(Dijmag, 1.5) / (std::pow(Sijmag, 2.5) + std::pow(Dijmag, 1.25));
}