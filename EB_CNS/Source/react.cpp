...
#endif
    {
      for (int nf = NUM_FIELD > 0 ? 1 : 0; nf <= NUM_FIELD; ++nf) 
      {
        ...
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
          ...
          if (T(i,j,k) <= 0.0) 
            std::cout << "Before reaction T=" << T(i,j,k)
              << " @ " << i << "," << j << "," << k << std::endl;
          ...
          const Real rEinew = snew_arr(i,j,k,UEDEN) 
...
// #if CNS_USE_EB
//           // mask(i,j,k) = (vfrac_arr(i,j,k) > 0.5) ? mask(i,j,k) : -1;          
//           mask(i,j,k) = (flag(i,j,k).numNeighbors() == 9) ? mask(i,j,k) : -1;
// #endif
        });
        ...
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
          if (mask(i,j,k) != -1) {
            ...
            // for (int n = 0; n < NUM_SPECIES; ++n)
            //   if (rY(i,j,k,n) < -1e-6 || rY(i,j,k,n) > 1.0 + 1e-6 || std::isnan(rY(i,j,k,n))) 
