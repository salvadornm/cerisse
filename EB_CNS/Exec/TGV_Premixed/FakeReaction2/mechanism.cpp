...
{
  for (int j = 0; j < 1; ++j) { _rmap[j] = rmap[j]; }
}
            ...
            amrex::Real q_f[], amrex::Real q_r[])
{
  ...
  for (int id = 0; id < 2; ++id) { c[id] = x[id] * PORT; }
...
void CKNCF(int* ncf)
{
  int kd = 1;
  // Zero ncf
  for (int id = 0; id < kd * 2; ++id) { ncf[id] = 0; }
...
void SPARSITY_INFO(int* nJdata, const int* consP, int NCELLS)
{
  ...
  for (int n = 0; n < 2; n++) { conc[n] = 1.0 / 2.000000; }
  aJacobian(Jac.data(), conc.data(), 1500.0, *consP);

  int nJdata_tmp = 0;
  for (int k = 0; k < 3; k++) {
    for (int l = 0; l < 3; l++) {
      if (Jac[3 * k + l] != 0.0) { nJdata_tmp = nJdata_tmp + 1; }
    }
  }
