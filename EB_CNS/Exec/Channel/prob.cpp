  ...
  const amrex::Real* probhi)
{
  ...
  amrex::Real csw;
  eos.RTY2Cs(CNS::h_prob_parm->rho_w, CNS::h_prob_parm->T_w, CNS::h_prob_parm->massfrac.begin(), csw);    
  ...
  CNS::h_prob_parm->f_x = CNS::h_prob_parm->rho_w * CNS::h_prob_parm->u_tau * CNS::h_prob_parm->u_tau 
                        / CNS::h_prob_parm->H / CNS::h_prob_parm->rho_b;

  amrex::Gpu::copy(amrex::Gpu::hostToDevice, CNS::h_prob_parm, 
                   CNS::h_prob_parm+1, CNS::d_prob_parm);
  ...
  trans_parm.viscosity_S = CNS::h_prob_parm->T_w * 0.463; // use this to approximate mu ~ T^0.7 in T/Tw between 1 and 1.25
