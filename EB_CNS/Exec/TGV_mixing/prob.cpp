                    ...
                    const amrex::Real* problo, const amrex::Real* probhi)
{
  ...
  trans_parm.const_diffusivity =
    trans_parm.const_viscosity / CNS::h_prob_parm->schmidt;
...
void CNS::full_prob_post_timestep(int /*iteration*/)
{
  ...
  if (level == 0) {
      ...
      auto reduce_tuple = amrex::ParReduce(
        ...
        [=] AMREX_GPU_DEVICE(int box_no, int i, int j,
          ...
          for (int nf = 1; nf <= NUM_FIELD; ++nf) {
            ...
            const amrex::Real Yf1 =
              sarrs[box_no](i, j, k, UFS + nf * NVAR) * rhofinv;
    ...
    if (amrex::ParallelDescriptor::IOProcessor()) {
      ...
      data_log << std::setw(datwidth) << std::setprecision(datprecision)
               << mixedness;
      data_log << std::setw(datwidth) << std::setprecision(datprecision) << msgs;
      data_log << std::endl;
    }
  }
}