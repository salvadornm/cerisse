                    ...
                    const amrex_real* /*problo*/, const amrex_real* /*probhi*/)
{
  ...
  CNS::h_prob_parm->rho_r =
    CNS::h_prob_parm->rho_l * (g + 1.0) * M_s * M_s / (2.0 + (g - 1.0) * M_s * M_s);
  CNS::h_prob_parm->u_r =
    CNS::h_prob_parm->u_l * (2.0 + (g - 1.0) * M_s * M_s) / ((g + 1.0) * M_s * M_s);
  CNS::h_prob_parm->p_r =
    CNS::h_prob_parm->p_l * (1.0 + (2.0 * g / (g + 1.0)) * (M_s * M_s - 1.0));
