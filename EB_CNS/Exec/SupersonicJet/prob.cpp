  ...
  const amrex::Real* probhi)
{
  ...
  eos.PYT2RE(CNS::h_prob_parm->p1, CNS::h_prob_parm->Y1.begin(), CNS::h_prob_parm->T1,
    CNS::h_prob_parm->rho1, CNS::h_prob_parm->ei1);
  eos.PYT2RE(CNS::h_prob_parm->p2, CNS::h_prob_parm->Y2.begin(), CNS::h_prob_parm->T2,
    CNS::h_prob_parm->rho2, CNS::h_prob_parm->ei2);
  eos.PYT2RE(CNS::h_prob_parm->p_a, CNS::h_prob_parm->Y_a.begin(), CNS::h_prob_parm->T_a,
    CNS::h_prob_parm->rho_a, CNS::h_prob_parm->ei_a);

  amrex::Real c1, c2;
  eos.RTY2Cs(CNS::h_prob_parm->rho1, CNS::h_prob_parm->T1, CNS::h_prob_parm->Y1.begin(), c1);
  eos.RTY2Cs(CNS::h_prob_parm->rho2, CNS::h_prob_parm->T2, CNS::h_prob_parm->Y2.begin(), c2);

  amrex::Print() << "Confirm inlet Mach number: M_air = "  << CNS::h_prob_parm->u1 / c1 << 
